...
static int ov2640_init_at_startup(struct gspca_dev *gspca_dev)
{
	...
	__raw_write(gspca_dev, 0x40, 3, 0x0000, 0x0200, 12, dat_init1);

	common(gspca_dev);

	__raw_read(gspca_dev, 0xc0, 2, 0x0000, 0x0006, 1, dat_init2);

	__raw_write(gspca_dev, 0x40, 1, 0x00ef, 0x0006, 0, NULL);

	__raw_read(gspca_dev, 0xc0, 2, 0x0000, 0x0000, 1, dat_init3);

	__raw_write(gspca_dev, 0x40, 1, 0x0051, 0x0000, 0, NULL);
...
static int ov2640_init_post_alt(struct gspca_dev *gspca_dev)
{
	...
	__raw_write(gspca_dev, 0x40, 3, 0x0000, 0x0200, 12, dat_post);
	common(gspca_dev);
	...
	case IMAGE_640:
		...
		__raw_write(gspca_dev, 0x40, 3, 0x0000, 0x0200, 12, dat_640);
		break;

	case IMAGE_800:
		n = fetch_validx(gspca_dev, tbl_800, ARRAY_SIZE(tbl_800));
		__raw_write(gspca_dev, 0x40, 3, 0x0000, 0x0200, 12, dat_800);
		break;
	...
	case IMAGE_1280:
		...
		if (reso == IMAGE_1280) {
			...
			__raw_write(gspca_dev, 0x40, 1, 0x601d, 0x0086, 0, NULL);
			__raw_write(gspca_dev, 0x40, 1, 0x6001, 0x00d7, 0, NULL);
			__raw_write(gspca_dev, 0x40, 1, 0x6082, 0x00d3, 0, NULL);
		}

		n = fetch_validx(gspca_dev, tbl_big_c, ARRAY_SIZE(tbl_big_c));

		if (reso == IMAGE_1280) {
			__raw_write(gspca_dev, 0x40, 1, 0x6001, 0x00ff, 0, NULL);
			__raw_write(gspca_dev, 0x40, 3, 0x0000, 0x0200,
					12, dat_1280);
		} else {
			__raw_write(gspca_dev, 0x40, 1, 0x6020, 0x008c, 0, NULL);
			__raw_write(gspca_dev, 0x40, 1, 0x6001, 0x00ff, 0, NULL);
			__raw_write(gspca_dev, 0x40, 1, 0x6076, 0x0018, 0, NULL);
			__raw_write(gspca_dev, 0x40, 3, 0x0000, 0x0200,
					12, dat_1600);
		}
		break;
	...
	__raw_read(gspca_dev, 0xc0, 2, 0x0000, 0x0000, 1, c50);
	keep_on_fetching_validx(gspca_dev, tbl_sensor_settings_common_b,
				ARRAY_SIZE(tbl_sensor_settings_common_b), n);
	__raw_read(gspca_dev, 0xc0, 2, 0x6000, 0x8004, 1, c28);
	keep_on_fetching_validx(gspca_dev, tbl_sensor_settings_common_b,
				ARRAY_SIZE(tbl_sensor_settings_common_b), n);
	__raw_read(gspca_dev, 0xc0, 2, 0x6000, 0x8004, 1, ca8);
	keep_on_fetching_validx(gspca_dev, tbl_sensor_settings_common_b,
				ARRAY_SIZE(tbl_sensor_settings_common_b), n);
	__raw_read(gspca_dev, 0xc0, 2, 0x0000, 0x0000, 1, c50);
	keep_on_fetching_validx(gspca_dev, tbl_sensor_settings_common_b,
...
static int ov2640_camera_settings(struct gspca_dev *gspca_dev)
{
	...
	if (backlight != sd->vold.backlight) {
		...
		__raw_write(gspca_dev, 0x40, 1, 0x6001                 , 0x00ff,
				0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x601f + backlight     , 0x0024,
				0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x601f + backlight - 10, 0x0025,
				0, NULL);
	...
	if (bright != sd->vold.brightness) {
		...
		__raw_write(gspca_dev, 0x40, 1, 0x6000         , 0x00ff, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6009         , 0x007c, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6000 + bright, 0x007d, 0, NULL);
	}
	...
	if (wbal != sd->vold.whitebal) {
		...
		__raw_write(gspca_dev, 0x40, 1, 0x6000       , 0x00ff, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6003       , 0x007c, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6000 + wbal, 0x007d, 0, NULL);
	}
	...
	if (cntr != sd->vold.contrast) {
		...
		__raw_write(gspca_dev, 0x40, 1, 0x6000       , 0x00ff, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6007       , 0x007c, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6000 + cntr, 0x007d, 0, NULL);
	}
	...
	if (sat != sd->vold.saturation) {
		...
		__raw_write(gspca_dev, 0x40, 1, 0x6000      , 0x00ff, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6001      , 0x007c, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6000 + sat, 0x007d, 0, NULL);
	}
	...
	if (sharp != sd->vold.sharpness) {
		...
		__raw_write(gspca_dev, 0x40, 1, 0x6000        , 0x00ff, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6001        , 0x0092, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x60c0 + sharp, 0x0093, 0, NULL);
	}
	...
	if (hue != sd->vold.hue) {
		...
		__raw_write(gspca_dev, 0x40, 1, 0x6000     , 0x00ff, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6002     , 0x007c, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6000 + hue * (hue < 255), 0x007d,
				0, NULL);
		if (hue >= sd->vmax.hue)
			sd->swapRB = 1;
		else
	...
	if (gam != sd->vold.gamma) {
		...
		__raw_write(gspca_dev, 0x40, 1, 0x6000      , 0x00ff, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6008      , 0x007c, 0, NULL);
		__raw_write(gspca_dev, 0x40, 1, 0x6000 + gam, 0x007d, 0, NULL);
	}
